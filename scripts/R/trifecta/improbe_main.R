...
imp_des_tib_csv <- file.path( opt$out_path, paste(opt$run_name, "improbe.order.manifest.csv.gz", sep='.') )
imp_ord_tab_csv <- file.path( opt$out_path, paste(opt$run_name, "improbe.order.stack.csv.gz", sep='.') )

if ( p2 )
  cat( glue::glue("{pmssg} Done. Building Improbe Design = {imp_des_tib_csv}.{RET2}") )
...
if ( p2 )
  cat( glue::glue("{pmssg} Writing Improbe Design = {imp_des_tib_csv}...{RET}") )

readr::write_csv( imp_des_tib, imp_des_tib_csv )
...
opt$bsp_tsv <- run_bsmap( ref_fas = ref_seqs$pvirPacbioDovetail,
                          can_fas = opt$ord_fas, 
