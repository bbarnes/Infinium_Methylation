...
par$version <- 17 # Starting pre-built Better...

# This build is meant for San Diego vs. Singapore analysis
# par$version <- 18 # Starting pre-built All...
  ...
  for ( rm_outliers in rm_outliers_vec ) {
    ...
    for ( exp_name in names(msa_ssh_list) ) {
      ...
      for ( sentrix_name in names(ssh_sel_list) ) {
        ...
        cur_out_name <- NULL
        cur_out_name <- paste0(sentrix_name,".",exp_name,"_",outliers_str)
        cur_negs_csv <- NULL
