...
if ( file.exists( all_data_rds ) &&
  ...
  for ( rm_outliers in rm_outliers_vec ) {
    ...
    for ( exp_name in names(sel_ssh_list) ) {
      ...
      #
      # [TBD]: For titration everything is reversed...
      #
      ...
      if ( FALSE ) {
        ...
        cur_dB_tib %>% dplyr::arrange( dB_med ) %>% head() %>% as.data.frame()
        cur_dB_tib %>% dplyr::filter( dB_pas_call ) %>% base::nrow()
        
        # Interesting data plot::
        cur_dB_tib %>% dplyr::mutate( wS_per = (wS_per / 10) %>% as.integer() ) %>% print_sum( vec = c("wS_per","db_pas_per") ) %>% print(n=1000)
      ...
      #
      if ( FALSE ) {
        cor_pas_val <- cor( x = cur_dB_tib$wS_per, 
...
if ( FALSE ) {
  ...
  sdf_paths <- NULL
  sdf_paths <- list.files( path = file.path( opt$out_path, "pre_sdf","rm-outliers" ), pattern = paste0("_","rm-outliers",".read_idat_pair_r.rds$"), recursive = TRUE, full.names = TRUE )
  probe_map <- NULL
  ...
  rep1_qc_cnt1 <- rep1_screen_tib %>% 
    dplyr::filter( Pass_Cnt + Fail_Cnt != Full_Cnt ) %>% base::nrow()
  
  # ( 239371 + 27415 ) = 266786 / 301819 = 0.8839271
  rep1_screen_sum <- NULL
  rep1_screen_sum <- rep1_screen_tib %>% 
    print_sum( vec = c("Pass_Per"), 
               vb=vb+3,vt=vt,tc=tc )
  
  # ( 230716 + 26432 ) = 257148 / 283133 = 0.9082233
  rep1_screen_sum1 <- NULL
  rep1_screen_sum1 <- rep1_screen_tib %>% 
  ...
  # ( 177787 + 91406 ) = 269193 / 301819 = 0.8919021
  uhmA_screen_sum <- NULL
  uhmA_screen_sum <- uhmA_screen_tib %>% 
    print_sum( vec = c("Pass_Per"), 
               vb=vb+3,vt=vt,tc=tc )
  
  # [cg] ( 177777 + 91263 ) = 269040 / 283133 = 0.9502248
  ...
  # ( 187200 + 85157 ) = 272357 / 301819
  uhm1_screen_sum <- NULL
  uhm1_screen_sum <- uhm1_screen_tib %>% 
    print_sum( vec = c("Pass_Per"), 
               vb=vb+3,vt=vt,tc=tc )
  
  # [cg] ( 187184 + 84790 ) = 271974 / 283133 = 0.9605874
  ...
  # ( 219361 + 55903 ) = 275264 / 301819
  uhm2_screen_sum <- NULL
  uhm2_screen_sum <- uhm2_screen_tib %>% 
    print_sum( vec = c("Pass_Per"), 
               vb=vb+3,vt=vt,tc=tc )

  # [cg] ( 219341 + 55662 ) = 275003 / 283133 = 0.9712856
