...
rm_tmp_list <- pre_sdf_list[["rm-outliers"]]



tmp_tib2 <- NULL
tmp_tib2 <- prefix_to_sdf( prefix = sentrix_list[[1]]$Sentrix_Path,
                           platform = "EPIC", 
                           manifest = all_man_tib %>% 
                             dplyr::filter( Manifest_Version == "B4" ) %>%
                             dplyr::mutate( 
                               M = dplyr::case_when( M==0 ~ NA_integer_, TRUE ~ M )
                               # col=dplyr::case_when( col=="2" ~ NA_character_, TRUE ~ col) ),
                             ),
                           
                           out_dir  = file.path( opt$out_path, "tmp" ),
                           run_tag  = opt$run_name,
                           reload   = opt$reload, 
                           reload_min = 10,
                           parallel = opt$parallel, 
                           vb=vb, vt=vt, tc=0, tt = tt ) %>% 
  mutate_sdf_simple( steps = "iDB" )

tmp_tib2 %>% tibble::as_tibble()
tmp_tib1 %>% tibble::as_tibble() %>% dplyr::filter( col != "2")


tmp_tib <- NULL
tmp_tib <- prefix_to_sdf( prefix = sentrix_list[[1]]$Sentrix_Path,
                          
                          platform = "EPIC", 
                          # manifest = all_man_tib %>% dplyr::filter( Manifest_Version == "B4" ), 
                          # controls = NULL, # all_man_tib %>% dplyr::filter( Manifest == "ctl" & Manifest_Version == "B4" ),
                          out_dir  = file.path( opt$out_path, "tmp" ),
                          run_tag  = opt$run_name,
                          reload   = opt$reload, 
                          reload_min = 10,
...
pre_sdf_list$`rm-outliers`[[3]] %>% dplyr::select( Probe_ID:mask ) %>% 
  dplyr::mutate( col = dplyr::case_when( col=="2" ~ NA_character_, TRUE ~ col) ) %>%
  as.data.frame() %>%
  sesame::SigDF( platform = "EPIC" ) %>% # tibble::as_tibble() %>%
  mutate_sdf_simple( steps = "D" ) %>%
  mutate_sdf_simple( steps = "B" ) %>% tibble::as_tibble() %>% tail()

cur_sdfs <- lapply( sentrix_list %>% head(n=2),
                    prefix_to_sdf, 
                    platform = "EPIC", 
                    manifest = exp_man_tib, 
                    controls = all_man_tib %>% dplyr::filter( Manifest == "ctl" & Manifest_Version == "B4" ), 
                    out_dir  = file.path( opt$out_path, "tmp" ),
                    run_tag  = opt$run_name,
                    reload   = opt$reload, 
                    reload_min = 10,
...
idat_tib <- NULL
idat_tib <- read_idat_pair_r( 
  prefix = sentrix_list[[1]]$Sentrix_Path,
  
  neg_vec = msa_neg_ctls_tib$Address %>% as.vector(),
  man_tib = dplyr::bind_rows( msa_man_tib,all_man_tib ),
  # ctl_tib = msa_all_ctls_tib,
  ctl_tib = all_man_tib %>% dplyr::filter( Manifest == "ctl" & Manifest_Version == "B4" ),
  ...
  out_dir = file.path( pre_sdf_path, paste0(sentrix_list[[1]]$Sentrix_Name) ), 
  run_tag = paste( sentrix_list[[1]]$Sentrix_Name,outliers_str, sep="_" ),
