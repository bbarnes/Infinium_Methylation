...
if ( FALSE ) {
  ...
  all_data_tib <- readr::read_rds( file = all_data_rds )
  
  exp_auto_ssh_tib <- NULL
  exp_auto_ssh_csv <- file.path( opt$out_path, paste0( opt$run_name, "_exp_auto_ssh.csv.gz") )
  exp_auto_ssh_tib <- readr::read_csv( file = exp_auto_ssh_csv, show_col_types = FALSE ) %>%
    clean_tib()
...
for ( rm_outliers in rm_outliers_vec ) {
  for ( exp_name in names(exp_ssh_list) ) {
    ...
    if ( is.null(all_beta_tibs[[sample_base]]) || 
         all_beta_tibs[[sample_base]] %>% base::ncol() < 3 ) next
    ...
    all_data_tib <- all_data_tib %>% dplyr::bind_rows( 
...
if ( TRUE ) {
  ...
  # readr::write_csv( x = all_data_tib, file = all_data_csv )
  
  all_data_rds <- file.path( opt$out_path, paste0( opt$run_name,"_all_data.rds" ) )
  # readr::write_rds( x = uhm_data_tib, file = all_data_rds, compress = "gz" )
  readr::write_rds( x = all_data_tib, file = all_data_rds, compress = "gz" )
...
if ( FALSE ) {
  ...
  exp_auto_ssh_sum1 <- exp_auto_ssh_tib %>%
