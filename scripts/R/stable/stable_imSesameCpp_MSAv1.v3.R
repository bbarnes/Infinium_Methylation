...
uhm_ssh_list <- dplyr::bind_rows(
  uhm_base_ssh_tib %>% dplyr::filter( Sample_Titration ==  0 | Sample_Titration ==  50 ) %>% dplyr::mutate( Exp_Str = "titration_0_50" ),
  uhm_base_ssh_tib %>% dplyr::filter( Sample_Titration ==  0 | Sample_Titration == 100 ) %>% dplyr::mutate( Exp_Str = "titration_0_100" ),
  uhm_base_ssh_tib %>% dplyr::filter( Sample_Titration == 50 | Sample_Titration == 100 ) %>% dplyr::mutate( Exp_Str = "titration_50_100" ),
  
  # cel_base_ssh_tib %>% dplyr::mutate( Exp_Str = paste("replication",Sample_Group,Sample_Base, sep="_") ) %>%
  #   dplyr::filter( Sample_Base == "HELA" ) %>% dplyr::filter( Sentrix_Name %>% stringr::str_starts("207675480006") & Sample_Input == 200 ),

  cel_base_ssh_tib %>% dplyr::mutate( Exp_Str = paste("replication",Sample_Group,Sample_Base,"50", sep="_") ) %>%
    dplyr::filter( Sample_Base == "HELA" ) %>% dplyr::filter( Source_Key == "R0" & Sample_Input == 50 ),
  cel_base_ssh_tib %>% dplyr::mutate( Exp_Str = paste("replication",Sample_Group,Sample_Base,"100", sep="_") ) %>%
    dplyr::filter( Sample_Base == "HELA" ) %>% dplyr::filter( Source_Key == "R0" & Sample_Input == 100 ),
  cel_base_ssh_tib %>% dplyr::mutate( Exp_Str = paste("replication",Sample_Group,Sample_Base,"200", sep="_") ) %>%
    dplyr::filter( Sample_Base == "HELA" ) %>% dplyr::filter( Source_Key == "R0" & Sample_Input == 200 ),
  
  cel_base_ssh_tib %>% dplyr::mutate( Exp_Str = paste("replication",Sample_Group,Sample_Base,"50", sep="_") ) %>%
    dplyr::filter( Sample_Base == "JURKAT" ) %>% dplyr::filter( Source_Key == "R0" & Sample_Input == 50 ),
  cel_base_ssh_tib %>% dplyr::mutate( Exp_Str = paste("replication",Sample_Group,Sample_Base,"100", sep="_") ) %>%
    dplyr::filter( Sample_Base == "JURKAT" ) %>% dplyr::filter( Source_Key == "R0" & Sample_Input == 100 ),
  cel_base_ssh_tib %>% dplyr::mutate( Exp_Str = paste("replication",Sample_Group,Sample_Base,"200", sep="_") ) %>%
    dplyr::filter( Sample_Base == "JURKAT" ) %>% dplyr::filter( Source_Key == "R0" & Sample_Input == 200 )
...
for ( rm_outliers in rm_outliers_vec ) {
  for ( exp_name in names(uhm_ssh_list) ) {
    all_beta_tibs <- NULL
    all_poob_tibs <- NULL
    all_negs_tibs <- NULL
    ...
    ssh_sel_tibs <- NULL
    ssh_sel_tibs <-     uhm_ssh_list[[exp_name]] %>% 
    ...
    for ( sentrix_name in names(ssh_sel_list) ) {
      ...
      cur_auto_ssh_tib <- readr::read_csv( file = cur_auto_ssh_csv, show_col_types = FALSE ) %>%
        clean_tib() %>%
        dplyr::mutate( Experiment = exp_name ) %>%
    ...
    if ( exp_name %>% stringr::str_starts("titration") ) {
      ssh_uhm_list <- NULL
      ...
      if ( exp_name == "titration_0_100" ) {
        cur_min_dB  <- 0.5
        cur_cmp_str <- "gt"
        cur_is_abs  <- FALSE
      }
      if ( exp_name == "titration_0_50" ) {
        cur_min_dB  <- 0.2
        cur_cmp_str <- "gt"
        cur_is_abs  <- FALSE
      }
      if ( exp_name == "titration_50_100" ) {
        cur_min_dB  <- 0.1
        cur_cmp_str <- "gt"
        cur_is_abs  <- FALSE
      }
...
if ( FALSE ) {
  ...
  box_pas_wS_ggg <- uhm_data_tib %>% 
    ...
    ggplot2::facet_grid( rows = vars(Remove_Outliers), cols = vars(Experiment) )
