...
hist_prb_tib <- NULL
hist_prb_tib <- dplyr::bind_rows(
  dplyr::select( man_v1_base_tib, AlleleA_ProbeSeq,AlleleB_ProbeSeq ),
  dplyr::select( man_h4_body_tib, AlleleA_ProbeSeq,AlleleB_ProbeSeq )
) %>% 
...
man_a1_cnts_tib <- NULL
man_a1_cnts_tib <- man_a1_body_tib %>% 
  dplyr::rename(
    Probe_ID = IlmnID,
    Loci_ID  = Name
  ) %>%
  dplyr::add_count( Loci_ID, name="Loci_Cnt" ) %>%
  dplyr::left_join( hist_prb_tib, by=c("AlleleA_ProbeSeq","AlleleB_ProbeSeq") ) %>%
  dplyr::mutate(
...
can_rank_tib <- NULL
can_rank_tib <- man_a1_cnts_tib %>%
  dplyr::group_by( Loci_ID ) %>%
  dplyr::mutate( Canonical_Rank = dplyr::row_number() ) %>%
  dplyr::ungroup()

#
# [Done]: Set Canonical_Rank == 1 to Base Loci_ID
#
can_call_tib <- NULL
can_call_tib <- can_rank_tib %>%
  dplyr::mutate(
    Full_ID  = Probe_ID,
...
tot_id_cnt <- can_call_tib %>% base::nrow()
add_id_cnt <- can_call_tib %>% dplyr::distinct( AddressA_ID,AddressB_ID ) %>% base::nrow()
seq_id_cnt <- can_call_tib %>% dplyr::distinct( AlleleA_ProbeSeq,AlleleB_ProbeSeq ) %>% base::nrow()
prb_id_cnt <- can_call_tib %>% dplyr::distinct( Probe_ID ) %>% base::nrow()
ful_id_cnt <- can_call_tib %>% dplyr::distinct( Full_ID ) %>% base::nrow()
loc_id_cnt <- can_call_tib %>% dplyr::distinct( Loci_ID ) %>% base::nrow()
...
# [TBD]: Can Probably Delete this now:: Point Proven
#
if ( FALSE ) {
  #
  # Question: Why Not make every Rep_Num == 1 with no suffix?
  #.  - See Below...
  #
  man_a1_body_cnt <- man_a1_body_tib %>% 
    dplyr::distinct( IlmnID ) %>% base::nrow()
  
  man_a1_uniq_tib <- NULL
  man_a1_uniq_tib <- man_a1_body_tib %>% 
    dplyr::mutate( 
      Full_ID  = IlmnID,
  ...
  man_a1_uniq_cnt <- man_a1_uniq_tib %>% 
    dplyr::distinct( Probe_ID ) %>% base::nrow()
  #
  # NOTE: Above is there case where you need the counts for Loci_ID/Loci_Cnt weighted by Historic_Cnt
  #
}
...
epicv2_loc_dir <- safe_mkdir( dir = file.path( opt$out_path, "manifests" ), recursive = TRUE )
epicv2_man_csv <- file.path( opt$top_path, "data/manifests/methylation/MethylationEPIC_v2-2/EPIC-8v2-0_A1.csv" )
epicv2_sel_csv <- file.path( epicv2_loc_dir, "EPIC-8v2-0_A1.select-cg-ch-nv-rs.csv" )
epicv2_all_csv <- file.path( epicv2_loc_dir, "EPIC-8v2-0_A1.select-all.csv" )

# epicv2_grp_cmd <- paste0( "cat ",epicv2_man_csv," | grep -e ^IlmnID -e ^cg -e ^rs > ",epicv2_sub_csv )
# cmd_res_str <- base::system( command = epicv2_grp_cmd, intern = TRUE, ignore.stdout = TRUE, ignore.stderr = TRUE )

# Here is the command::
#. cat /Users/bbarnes/Documents/data/manifests/methylation/MethylationEPIC_v2-2/EPIC-8v2-0_A1.csv | grep -e ^IlmnID -e ^cg -e ^ch -e ^nv -e ^rs | cut -d, -f 1-23 > /Users/bbarnes/Documents/scratch/stable_scratch_EPICv2_docker_snps/EPICv2-UCSC-v1/manifests/EPIC-8v2-0_A1.select-cg-rs.csv

cat( glue::glue("cat {epicv2_man_csv} | cut -d, -f 1-23 >  ") )

epicv2_sel_tib <- NULL
if ( epicv2_sel_csv %>% file.exists() ) epicv2_sel_tib <- 
  readr::read_csv( file = epicv2_sel_csv, show_col_types = FALSE ) %>% 
  dplyr::filter( CHR!="0" ) %>%
  dplyr::filter( MAPINFO != 0 ) %>%
  clean_tib() %>%
  dplyr::arrange( CHR,MAPINFO,IlmnID )

epicv2_sel_sum <- NULL
epicv2_sel_sum <- epicv2_sel_tib %>% 
  dplyr::group_by( Probe_Type ) %>% 
  dplyr::summarise( Count=n(), .groups = "drop" )
if ( p4 ) epicv2_sel_sum %>% print( n=base::nrow(epicv2_sel_sum) )
...
epicv2_sel_vec <- NULL
epicv2_sel_vec <- c( "IlmnID","Name","Probe_Type",
                     "Strand_FR","Strand_TB","Strand_CO",
                     "Infinium_Design","Rep_Num",
                     "CHR","MAPINFO" )

epicv2_col_vec <- NULL
epicv2_col_vec <- c( "Probe_ID","Loci_ID","Probe_Type",
                     "Strand_FR","Strand_TB","Strand_CO",
                     "Infinium_Design","Rep_Num",
                     "Chromosome_hg38","Coordinate_hg38" )

epicv2_tar_tib <- NULL
epicv2_tar_tib <- epicv2_sel_tib %>% 
  dplyr::select( dplyr::all_of(epicv2_sel_vec) ) %>%
  magrittr::set_names( value = epicv2_col_vec ) %>%
  dplyr::mutate(
    Chromosome_hg38 = Chromosome_hg38 %>% 
...
epicv2_tar_din <- NULL
epicv2_tar_din <- epicv2_tar_tib %>% split(.$Probe_Type )

for ( din in names(epicv2_tar_din) ) {
  din <- "cg"
  if ( p2 ) cat(glue::glue("{pmssg} Parsing Probe_Type='{din}'... "))
  
  bed_tib <- NULL
  cur_tib <- NULL
  cur_tib <- epicv2_tar_din[[din]] %>%
    dplyr::arrange( Chromosome_hg38,Coordinate_hg38,Probe_ID )
  
  if ( din == "cg" ) {
    cur_tib <- cur_tib %>% 
      dplyr::filter( Infinium_Design == 1 ) %>%
      dplyr::mutate( 
        Next_Base_Beg = dplyr::case_when(
          Strand_FR == "F" ~ Coordinate_hg38 + 1, 
          Strand_FR == "R" ~ Coordinate_hg38 - 2, 
          TRUE ~ NA_real_ ),
        Next_Base_End = Next_Base_Beg + 1
      )
    
    bed_tib <- cur_tib %>% 
      dplyr::select( Chromosome_hg38,Next_Base_Beg,Next_Base_End,Probe_ID,strand )
  }
  if ( din == "ch" ) {
  ...
  cur_cnt <- cur_tib %>% base::nrow()
  if ( p2 ) cat(glue::glue("Count='{cur_cnt}'.{RET}"))
  if ( p4 ) cur_tib %>% head(n=3) %>% print()
  if ( p2 ) cat(glue::glue("{RET2}"))
  
  # Current Temp Files::
  cur_bed_tsv <- file.path( opt$probe_bed_dir, paste0("probe.",din,".bed.tsv.gz") )
  cur_vcf_tsv <- file.path( opt$probe_vcf_dir, paste0("probe.",din,".vcf.tsv") )
  snp_vcf_tsv <- file.path( opt$dbSNP_vcf_dir, paste0("probe.",din,".vcf.tsv") )
  
  # Write BED::
  readr::write_tsv( x = bed_tib, file = cur_bed_tsv, col_names = FALSE )
  ...
  tabix_cmd <- paste0("tabix -h -R ",cur_bed_tsv," ",opt$dbSNP_151_vcf," > ",snp_vcf_tsv)
  
  # Tabix dbSNP:: Run
  base::system( tabix_cmd )
  
  vcf_tib <- NULL
  vcf_tib <- readr::read_tsv( file = snp_vcf_tsv, 
                              skip = 56, show_col_types = FALSE) %>% 
    magrittr::set_names( c("Chromosome_hg38","Coordinate_hg38","Target_ID",
                           "REF","ALT","QUAL","FILTER","INFO") ) %>% 
    dplyr::filter( stringr::str_length(REF) == 1 ) %>%
    # This removes most large indels::
    dplyr::filter( ALT %>% stringr::str_length() <= 5 ) %>%
    dplyr::filter( REF != ALT ) %>%
    dplyr::filter( !Target_ID %>% stringr::str_detect(",") ) %>% 
    dplyr::distinct( Chromosome_hg38,Coordinate_hg38,Target_ID,REF,ALT ) %>%
    clean_tib() %>% 
    dplyr::mutate( ALT_LenT = ALT %>% stringr::str_length(), 
                   ALT_LenC = ALT %>% stringr::str_remove_all(",") %>% stringr::str_length(),
  ...
  #
  prb_tar_tib <- NULL
  prb_tar_tib <- dplyr::bind_rows(
    dplyr::inner_join( vcf_tib, bed_tib, 
                       by=c("Chromosome_hg38","Coordinate_hg38"="Next_Base_Beg"), 
                       multiple = "all" ) %>% 
      dplyr::select( -Next_Base_End ) %>% dplyr::mutate( Dirc="B"),
    dplyr::inner_join( vcf_tib, bed_tib, 
                       by=c("Chromosome_hg38","Coordinate_hg38"="Next_Base_End"), 
                       multiple = "all" ) %>% 
      dplyr::select( -Next_Base_Beg ) %>% dplyr::mutate( Dirc="E")
  ) %>%
    dplyr::mutate( Loci_ID = Probe_ID %>% stringr::str_remove("_.*$"),
                   Chromosome_hg38 = paste0("chr",Chromosome_hg38) ) %>%
    dplyr::select( Chromosome_hg38,Coordinate_hg38,strand,
                   Probe_ID,Loci_ID,Target_ID, REF,ALT, 
                   ALT_LenT,ALT_LenC,ALT_LenD, Dirc ) %>%
    dplyr::arrange( Chromosome_hg38,Coordinate_hg38,Dirc,Probe_ID )
  
  prb_tar_sum <- NULL
  prb_tar_sum <- prb_tar_tib %>% 
    dplyr::group_by( Dirc,ALT_LenT,ALT_LenC,ALT_LenD ) %>%
    dplyr::summarise( Count=n(), .groups = "drop" )
  if ( p2 ) prb_tar_sum %>% print( n=base::nrow(prb_tar_sum) )


  validate_annoI <- TRUE
  validate_annoI <- FALSE
  if ( validate_annoI ) {
    #
    # [ERROR]: Here is the killer::
    #
    par_tar_mis <- NULL
    par_tar_mis <- prb_tar_tib %>% 
      dplyr::inner_join( annoI_tib, 
                         by=c("Loci_ID"), 
                         suffix=c("_v2","_v1") ) %>% 
      # dplyr::distinct( Loci_ID,
      #                  .keep_all = TRUE ) %>% head() %>% 
      dplyr::group_by( Dirc,ALT_LenT,ALT_LenC,ALT_LenD ) %>%
      dplyr::summarise( Count=n(), .groups = "drop" )
    ...
    par_tar_mis <- prb_tar_tib %>% 
      dplyr::inner_join( annoI_tib, 
                         by=c("Loci_ID","Target_ID",
                              "REF","ALT","strand",
                              "Chromosome_hg38"="Chromosome_CpG_hg19"), 
                         suffix=c("_v2","_v1") ) %>% 
      # dplyr::distinct( Loci_ID,
      #                  .keep_all = TRUE ) %>% head() %>% 
      dplyr::group_by( Dirc,ALT_LenT,ALT_LenC,ALT_LenD ) %>%
    ...
    prb_tar_tib %>% 
      dplyr::inner_join( annoI_tib, 
                         by=c("Loci_ID"), 
                         suffix=c("_v2","_v1") ) %>% 
      dplyr::filter( Dirc == "B" & ALT_LenT == 1 & ALT_LenC == 1 & ALT_LenD == 0 ) %>% as.data.frame()

    #
    # [DONE]: Check Coordinats Against annoI
    #
    
    # 420
    mat_cnt1 <- prb_tar_tib %>% 
      dplyr::inner_join( annoI_tib, 
                         by=c("Loci_ID"), 
                         suffix=c("_v2","_v1") ) %>% 
      dplyr::distinct( Loci_ID,
                       .keep_all = TRUE ) %>% base::nrow()
    
    
    # 417
    mat_cnt2 <- prb_tar_tib %>% 
      dplyr::inner_join( annoI_tib, 
                         by=c("Loci_ID","Target_ID"), 
                         suffix=c("_v2","_v1") ) %>% 
      dplyr::distinct( Loci_ID,Target_ID,
                       .keep_all = TRUE ) %>% base::nrow()
    
    # 420
    mat_cnt3 <- prb_tar_tib %>% 
      dplyr::inner_join( annoI_tib, 
                         by=c("Loci_ID","REF"), 
                         suffix=c("_v2","_v1") ) %>% 
      dplyr::distinct( Loci_ID,
    ...
    if ( p2 ) cat(glue::glue("{pmssg} Match Count 1 = '{mat_cnt1}'{RET}"))
    if ( p2 ) cat(glue::glue("{pmssg} Match Count 2 = '{mat_cnt2}'{RET}"))
    if ( p2 ) cat(glue::glue("{pmssg} Match Count 3 = '{mat_cnt3}'{RET}"))
    if ( p2 ) cat(glue::glue("{pmssg} Match Count 4 = '{mat_cnt4}'{RET}"))
    if ( p2 ) cat(glue::glue("{pmssg} Match Count 5 = '{mat_cnt5}'{RET}"))
    if ( p2 ) cat(glue::glue("{pmssg} Match Count 6 = '{mat_cnt6}'{RET}"))
    if ( p2 ) cat(glue::glue("{pmssg} Match Count 7 = '{mat_cnt7}'{RET}"))
    if ( p2 ) cat(glue::glue("{pmssg} Match Count 8 = '{mat_cnt8}'{RET}"))
...
epicv2_cpg_tib <- NULL
epicv2_cpg_tib <- epicv2_sel_tib %>%
  dplyr::filter( Infinium_Design == 1 ) %>%
  dplyr::filter( Probe_Type == "cg" )

epicv2_snp_tib <- NULL
epicv2_snp_tib <- epicv2_sel_tib %>%
  # dplyr::filter( Infinium_Design == 1 ) %>%
  dplyr::filter( Probe_Type == "rs" )

#
# We Need to Extract annoI and format annoS
#
