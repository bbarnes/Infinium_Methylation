...
man_body_tib <- NULL
man_body_tib <- rank_historic_probes( 
  tib = man_a1_body_tib, 
...
# [TBD]: Intersect all at once...
# [TBD]: Update intersect_bed_tabix()
...
# [TBD]: Implement screening inside intersect_bed_tabix() with 'Match Types'
# [TBD]: Implement matching by REF
# [TBD]: Re-run everything but with EPICv1
# [TBD]: Add "EPICv1" to run names...
...
#
# [TBD]: Fix the naming below...
#
nxb_up_tib <- NULL
nxb_up_tib <- tabix_tib %>%
  dplyr::inner_join( 
    dplyr::mutate( man_body_tib, Chromosome = Chromosome %>% stringr::str_remove("^chr") ),
    by=c("Chromosome","Coordinate"="Nxb_Pos_Up"),
    multiple = "all"
  )
cpg_up_tib <- NULL
cpg_up_tib <- tabix_tib %>%
  dplyr::inner_join( 
    dplyr::mutate( man_body_tib, Chromosome = Chromosome %>% stringr::str_remove("^chr") ),
...
#
# [TBD]: Reduce all the extra work done above...
#
d0_matI_tib %>% dplyr::select( 1:11,Nxb_Nuc_Up,Cpg_Nuc_Up,Cpg_Nuc_Dn,Nxb_Nuc_Dn ) %>% dplyr::filter( REF.x != Nxb_Nuc_Dn )

#
# [TBD]: Investigate MAF screening and removing low MAF
#. - Done before...
#
#  > opt$dbSNP_151_vcf
#  [1] "/Users/bbarnes/Documents/data/dbSNP/NCBI/common_all_20180418.vcf.gz"
#
...
# Sanity Check:: Just Checking...
# can_call_tib %>% dplyr::filter( is.na(Probe_Type) )

can_call_din <- NULL
can_call_din <- man_body_tib %>% split(.$Probe_Type )



for ( din in names(can_call_din) ) {
  din <- "cg"
  if ( p2 ) cat(glue::glue("{pmssg} Parsing Probe_Type='{din}'... "))
  
  bed_tib <- NULL
  cur_tib <- NULL
  cur_tib <- can_call_din[[din]]
  ...
  if ( din == "cg" ) {
    ...
    #
    # [DONE]: Investigate CHR==0 probes by type. Conclusion: Not an issues...
    #
    bed_tib <- cur_tib %>% 
      dplyr::filter( Chromosome_hg38 != "0" ) %>%
      dplyr::filter( Next_Base_Beg > 0 ) %>%
      dplyr::filter( Next_Base_End > 0 ) %>%
  ...
  # Tabix dbSNP:: Cmd
  # [RM]: epicv2_int_dbSNP_RG_vcf <- file.path( opt$top_path, "Projects.new/EPIC_v2/docker/intersect/EPICv2.dbSNP151-common.next-base-RG.vcf" )
